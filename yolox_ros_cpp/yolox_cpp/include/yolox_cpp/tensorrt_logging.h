...
class LogStreamConsumerBuffer : public std::stringbuf {
  public:
    ...
    operator=(const LogStreamConsumerBuffer &) = delete;
    LogStreamConsumerBuffer &operator=(LogStreamConsumerBuffer &&) = delete;

    ~LogStreamConsumerBuffer() override {
        // std::streambuf::pbase() gives a pointer to the beginning of the
        // buffered part of the output sequence std::streambuf::pptr() gives a
        // pointer to the current position of the output sequence if the pointer
        // to the beginning is not equal to the pointer to the current position,
    ...
    //! synchronizing the stream buffer consists of inserting the buffer
    //! contents into the stream, resetting the buffer and flushing the stream
    //!
    ...
    void putOutput() {
        if (mShouldLog) {
            ...
            mOutput << std::setw(2) << std::setfill('0') << tm_local->tm_mday
                    << "/";
            mOutput << std::setw(4) << std::setfill('0')
            ...
            // insert the buffer contents pre-appended by the appropriate prefix
            // into the stream
            mOutput << mPrefix << str();
        }
...
class Logger : public nvinfer1::ILogger {
  public:
    ...
    //! associated with this Logger \return The nvinfer1::ILogger associated
    //! with this Logger
    //!
    ...
    //!
    //! \brief Opaque handle that holds logging information for a particular
    //! test
    //!
    //! This object is an opaque handle to information used by the Logger to
    //! print test results. The sample must call Logger::defineTest() in order
    //! to obtain a TestAtom that can be used with
    ...
    class TestAtom {
      ...
      private:
        ...
        TestAtom(bool started, const std::string &name,
  ...
  private:
    //!
    //! \brief returns an appropriate string for prefixing a log message with
    //! the given severity
    //!
    ...
    //!
    //! \brief returns an appropriate output stream (cout or cerr) to use with
    //! the given severity
    //!
    static std::ostream &severityOstream(Severity severity) {
        return severity >= Severity::kINFO ? std::cout : std::cerr;
