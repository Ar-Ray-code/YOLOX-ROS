...
YoloXONNXRuntime::YoloXONNXRuntime(const file_name_t &path_to_model,
                                   ...
                                   int inter_op_num_threads, bool use_cuda,
                                   int device_id, bool use_parallel,
    ...
    : AbcYoloX(nms_th, conf_th, model_version, num_classes, p6),
      ...
      inter_op_num_threads_(inter_op_num_threads), use_cuda_(use_cuda),
      device_id_(device_id), use_parallel_(use_parallel) {
    try {
        ...
        this->session_ =
            Ort::Session(this->env_, path_to_model.c_str(), session_options);
    ...
    this->input_name_ =
        std::string(this->session_.GetInputNameAllocated(0, ort_alloc).get());
    ...
    this->output_name_ =
        std::string(this->session_.GetOutputNameAllocated(0, ort_alloc).get());
