...
extern "C" void launchGPUResizeAndBlobFromImage(uchar3 *image_data,
                                                float *output, int r_width,
                                                int r_height, int input_width,
                                                int input_height, float w_ratio,
                                                float h_ratio,
                                                cudaStream_t stream);
...
YoloXTensorRT::YoloXTensorRT(const file_name_t &path_to_engine, int device,
    ...
    this->runtime_ =
        std::unique_ptr<IRuntime>(createInferRuntime(this->gLogger_));
    assert(this->runtime_ != nullptr);
    this->engine_ = std::unique_ptr<ICudaEngine>(
        this->runtime_->deserializeCudaEngine(trtModelStream.data(), size));
    assert(this->engine_ != nullptr);
    this->context_ = std::unique_ptr<IExecutionContext>(
    ...
    assert(this->engine_->getTensorDataType(input_name) ==
           nvinfer1::DataType::kFLOAT);
    assert(this->engine_->getTensorDataType(output_name) ==
           nvinfer1::DataType::kFLOAT);

    // Create GPU buffers on device
    CHECK(cudaMalloc(&this->inference_buffers_[this->inputIndex_],
                     3 * this->input_h_ * this->input_w_ * sizeof(float)));
    CHECK(cudaMalloc(&this->inference_buffers_[this->outputIndex_],
    ...
    assert(this->context_->setInputTensorAddress(
        input_name, this->inference_buffers_[this->inputIndex_]));
    assert(this->context_->setOutputTensorAddress(
        output_name, this->inference_buffers_[this->outputIndex_]));
...
std::vector<yolox_cpp::Object>
YoloXTensorRT::inference(const cv::Mat &frame, uchar3 *d_image, float *d_output,
                         cudaStream_t copy_stream_,
                         cudaStream_t resize_stream_) {
    auto t0 = std::chrono::high_resolution_clock::now();

    cudaMemcpy2DAsync(d_image, frame.cols * sizeof(uchar3), frame.data,
                      frame.step, frame.cols * sizeof(uchar3), frame.rows,
    ...
    launchGPUResizeAndBlobFromImage(d_image, d_output, this->input_w_,
                                    this->input_h_, frame.cols, frame.rows, w_r,
                                    h_r, resize_stream_);
    ...
    const float scale =
        std::min(static_cast<float>(this->input_w_) / frame.cols,
...
void YoloXTensorRT::doInference(const float *input, float *output,
    ...
    // DMA output back to host Change to Device to Device copy
    ...
    CHECK(cudaMemcpyAsync(output, this->inference_buffers_[this->outputIndex_],
                          this->output_size_ * sizeof(float),
